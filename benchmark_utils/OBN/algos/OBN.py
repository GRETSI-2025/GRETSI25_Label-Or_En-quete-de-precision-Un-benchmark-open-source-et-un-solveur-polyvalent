...
class OBN(base):
    ...
    def compute(self, S, A0, status_f, history, test_check_f):
        ...
        if A0 is None:
            ...
            A_diag = np.ones(self.N, dtype='float32')
        ...
        if status_f is not None:
            status.append(status_f(A, 0.0))

        lam_matrix = lam*np.ones_like(A)  # new
        np.fill_diagonal(lam_matrix, 0)  # new
        ...
        for t in range(self.T):
            ...
            if init_step != 0:
                ...
                G_min = np_soft_threshold(
                    G + lam_matrix*sign_A, lam_matrix*(1.0 - mask_A))
                # sign_soft_G = np.sign(
                #     np_soft_threshold(G, lam), dtype='float32')
                sign_soft_G = np.sign(
                    np_soft_threshold(G, lam_matrix), dtype='float32')
                ...
                A, step = projected_linesearch_F(
                    A, S, lam, G_min, Z, X, init=init_step, max_iter=self.ls_iter, step_lim=self.step_lim)
                if step == 0:
                    init_step = 0
